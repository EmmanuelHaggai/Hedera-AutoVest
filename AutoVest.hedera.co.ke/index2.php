...
// Get query params
$amountParam = isset($_GET['amount']) ? $_GET['amount'] : '0.00';
$refCode     = isset($_GET['ref']) ? $_GET['ref'] : '';

// Sanitize incoming amount for display only
$amountDisplay = htmlspecialchars($amountParam, ENT_QUOTES, 'UTF-8');
...
$status           = 'pending';
$statusMessage    = '';
$whatsappLinkHref = 'https://wa.me/18167908575'; // fallback
...
if ($refCode !== '') {
    ...
    if ($db) {
        ...
        if ($row) {
            ...
            if ($status === 'paid') {
                $statusMessage = 'Your card payment has been confirmed and your wallet top-up is being finalized.';
...
// Build extra detail line about original vs charged amount
$detailAmountLine = '';
if ($linkValid && $originalAmount !== null) {
    $charged     = (float)$amountParam;
    $orig        = (float)$originalAmount;
    $chargedFmt  = number_format($charged, 2, '.', '');
    $originalFmt = number_format($orig, 2, '.', '');

    if (abs($charged - $orig) < 0.01) {
        $detailAmountLine = "Requested and charged amount: <strong>\${$originalFmt}</strong>.";
    } else {
        $detailAmountLine =
            "Requested amount: <strong>\${$originalFmt}</strong>.<br>" .
            "Card was charged: <strong>\${$chargedFmt}</strong>.";
    }
}
?><!doctype html>
<html lang="en">
                    ...
                    <div class="header-inner-wrapper">
                        <div class="navigation-menu-wrapper menu-b538310 desktop-wrapper">
                            ...
                            <ul id="menu-main-menu" class="navigation-menu desktop">
                                ...
                                <li class="menu-item">
                                    <a href="https://autovest.hedera.co.ke/demo/" target="_blank">Watch Demo</a>
                                </li>
                            </ul>
                        </div>
